...
<td>Home deface:</td>
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE special = '1' AND home = '1'");
$row = $q->fetchArray();
$totalhm = $row[0];
...
<tr>
<td>Special Onhold:</td>
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '1' AND special = '1'");
$row = $q->fetchArray();
$totaloh = $row[0];
...
$offset = ($phal - 1) * $hal;
$halno = 1+$offset;
$q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.special = '1' ORDER BY date DESC LIMIT $offset, $hal");
$q1 = $db->query("SELECT COUNT(*) FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.special = '1'");
$th = $q1->fetchArray();
$thal = $th[0];
...
<table>
<tbody>
<tr><th>Tanggal</th><th>Attacker</th><th>Team</th><th>Onhold</th><th>Home</th><th>URL</th><th>WEB IP</th><th>Mirror</th></tr>";
...
$defid = $row['defacer'];
echo "<tr><td>$date</td><td><a href=\"attacker.php?id=$defid\">$username</a></td><td>$team<td class=\"acenter\">$status</td><td class=\"acenter\">$home</td><td>$site</td><td>$ip</td><td class=\"acenter\"><a href=\"mirror.php?id=$mirrorid\">Lihat</td></tr>
";
$halno++;
} // while
echo "</tbody>
