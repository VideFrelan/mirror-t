...
$defid = $_GET['id'];
if (($defid) && (is_numeric($defid))) {
$defid = $defid; // dummy
} else {
$defid = 0;
}
$q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$defid'");
$row = $q->fetchArray();
$defname = $row['username'];
$judul = "Attacker $defname";
include 'aset/kepala.php';
include 'aset/heker.php';
?>
<table style="width:100px !important">
<tr><th colspan="3" nowrap>Statistik attacker <?php echo $defname; ?></th></tr>
<tr>
<td>Total deface</td><td class="acenter">:</td>
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE mirror.defacer = '$defid'");
$row = $q->fetchArray();
$totalarc = $row[0];
...
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '0' AND mirror.defacer = '$defid'");
$row = $q->fetchArray();
$totalarc = $row[0];
echo "<td class=\"acenter\">".$totalarc."</td>";
?>
...
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '1' AND mirror.defacer = '$defid'");
$row = $q->fetchArray();
$totalarc = $row[0];
echo "<td class=\"acenter\">".$totalarc."</td>";
?>
...
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE home = '1' AND mirror.defacer = '$defid'");
$row = $q->fetchArray();
$totalhm = $row[0];
...
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE special = '1' AND mirror.defacer = '$defid'");
$row = $q->fetchArray();
$totalspc = $row[0];
...
<b>Mirror attacker <?php echo $defname; ?>:</b><br>
<?php
$hal = 30;
$halid = $_GET['hal'];
...
$offset = ($phal - 1) * $hal;
$halno = 1+$offset;
$q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$defid' ORDER BY date DESC LIMIT $offset, $hal");
$q1 = $db->query("SELECT COUNT(*) FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$defid'");
$th = $q1->fetchArray();
$thal = $th[0];
...
$teamid = $row['team'];
echo "<tr><td>$date</td><td>$username</td><td><a href=\"team.php?id=$teamid\">$team</a></td><td class=\"acenter\">$status</td><td style=\"text-align:center !important;\">$special</td><td style=\"text-align:center !important;\">$home</td><td>$site</td><td>$ip</td><td style=\"text-align:center !important;\"><a href=\"mirror.php?id=$mirrorid\">Lihat</td></tr>
";
$halno++;
} // while
echo "</tbody>
...
echo " <a href=\"?id=$defid&hal=".($halid-1)."\"><span><<</span></a>"; 
}
}
...
if ($i == $totalhal) {
echo " <a href=\"?id=$defid&hal=$totalhal\">" . $totalhal . "</a>";
}
else if ($i != $halid){
echo " <a href=\"?id=$defid&hal=$i\"> " . $i . "</a>";
$jhal = $i;
} else {
echo " <b>$i</b>";
}
