...
<td>Home deface:</td>
<?php
$q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '1' AND home = '1'");
$row = $q->fetchArray();
$totalhm = $row[0];
...
$offset = ($phal - 1) * $hal;
$halno = 1+$offset;
$q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.status = '1' ORDER BY date DESC LIMIT $offset, $hal");
$q1 = $db->query("SELECT COUNT(*) FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.status = '1'");
$th = $q1->fetchArray();
$thal = $th[0];
